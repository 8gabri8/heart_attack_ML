...
def logistic_regression(y, tx, initial_w, max_iters, gamma):
    ...
    # Initialize weights
    w = initial_w
    # ensure max_iters is a positive integer
    ...
    for n_iter in range(max_iters):
        ...
        print(
            f"Iteration {n_iter+1}/{max_iters}, Gradient: {grad}, Loss: {loss}, Weights: {w.flatten()}"
        )
...
def reg_logistic_regression(y, tx, lambda_, initial_w, max_iters, gamma):
    ...
    if max_iters == 0:
        loss, _ = calculate_loss_grad_penalized_logistic_regression(y, tx, w, lambda_)
        return w, loss

    for n_iter in range(max_iters):
        # Compute the loss and gradient
        _, gradient = calculate_loss_grad_penalized_logistic_regression(
            y, tx, w, lambda_
        )
        ...
        # Print progress (optional)
        print(
            f"Iteration {n_iter + 1}/{max_iters}, Loss: {loss:.4f}, Weights: {w.flatten()}"
        )
