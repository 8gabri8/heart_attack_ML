...
def main():
    ...
    parser.add_argument(
        "InputDir", type=str, help="Path to the folder containing the dataset."
    )
    ...
    NORMALIZATION_METHOD = (
        "z-score"  # Method to normalize continuous features during preprocessing
    )
    ...
    PREDICTION_METHOD = (
        "least_squares"  # Prediction method (see training_function.py for options)
    )
    ...
    SPECIFIC_FEATURES = (
        ["_AGE80", "_BMI5"],
    )  # Subset of feauture to use, ignoring all the others (in case is choosen specfiic type of feature engeneering)
    ...
    x_train, x_test, y_train, train_ids, test_ids = load_csv_data(
        DATASET_DIR, sub_sample=False
    )
    print("Dataset Loaded.")

    # Extract the columns name (feature name)
    feature_name = np.genfromtxt(
        os.path.join(DATASET_DIR, "x_train.csv"), delimiter=",", dtype=str, max_rows=1
    )
    feature_name = feature_name[
        1:
    ]  # The "ids" cols have been removed in the load_csv_data() fucntion
