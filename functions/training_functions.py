...
def process_train_data(
    ...
    structured_array_x_train = rfn.repack_fields(
        structured_array_x_train[specific_features]
    )
    ...
    if is_train_set:
        ...
        columns_to_keep = [
            name
            for name in structured_array_x_train.dtype.names
    ...
    for i, col_name in enumerate(cols_name):
        ...
        if type_feature == "continous":
            ...
            if nan_symbol == "nan":
                ...
                col_data[
                    col_data == nan_symbol
                ] = nan_symbol  # Replace with predefined missing value
    ...
    nan_columns = [
        ...
        if np.isnan(structured_array_x_train[col_name]).any()
    ]
    ...
    if plot_distributions:
        ...
        for i in range(num_columns):
            axes[i].hist(
                structured_array_x_train[feature_names[i]],
                bins=150,
                alpha=0.7,
                color="blue",
                edgecolor="black",
    ...
    discrete_keys = [
        ...
        if value[1] == "discrete" and key in structured_array_x_train.dtype.names
    ]
    continous_keys = [
        key
        for key, value in data_dict.items()
        if value[1] == "continous" and key in structured_array_x_train.dtype.names
    ]
    ...
    if x_train_con_norm.size != 0:
        x_train_con_np = np.column_stack(
            [x_train_con_norm[field] for field in x_train_con_norm.dtype.names]
        )
    if x_train_dis_enc.size != 0:
        x_train_dis_np = np.column_stack(
            [x_train_dis_enc[field] for field in x_train_dis_enc.dtype.names]
        )
    ...
    if use == "all":  # All features
        ...
        x_train_final = np.concatenate(
            [build_poly(x_train_con_np, degree=degree_pol), x_train_dis_np], axis=1
        )
...
def stratified_k_fold_cross_validation(
    ...
    Args:
        ...
        prediction_method (str): Optimization method to use. Options are:
            ...
            - "mae_gd": NEED TO IMPLEMENT
            - "ridge_gd": Ridge (linear regression + mse + L2) with Gradient Descent optimizer
            - "ridge_adam": NEED TO IMPLEMENT
            - "lasso_gd": NEED TO IMPLEMENT
    ...
    if not np.array_equal(unique_labels, [-1, 1]):
        raise ValueError(
            f"Invalid labels in y. Expected labels to be +1 and -1, but found: {unique_labels}"
        )
    ...
    for class_label, indices in class_indices.items():
        ...
        for i in range(
            len(indices) % k_folds
        ):  # those remaining samples are distributed across the first few folds.
        ...
        for fold_index in range(k_folds):
            folds[fold_index].extend(
                indices[current_index : current_index + fold_sizes[fold_index]]
            )
    ...
    for fold in range(k_folds):
        ...
        if initialization_w not in valid_initializations:
            raise ValueError(
                f"Invalid initialization method: {initialization_w}. "
        ...
        if prediction_method not in valid_methods:
            raise ValueError(
                f"Invalid optimization method: {prediction_method}. "
        ...
        if prediction_method == "mse_gd":
            w, train_loss = mean_squared_error_gd(
                y_train_fold, x_train_fold, initial_w, max_iters, gamma
            )
        elif prediction_method == "ridge_gd":
            w, train_loss = ridge_regression(
                y_train_fold, x_train_fold, lambda_=lambda_regularization
            )
        ...
        val_error = y_val_fold - np.dot(
            x_val_fold, w
        )  # Validation error = true_label - predictions
        ...
        # Print fold metrics
        print(
            f"Fold {fold + 1}/{k_folds}: Validation Loss={val_loss}, Accuracy={val_accuracy}, F1={val_f1_score}"
        )
        print(
            f"Fold {fold + 1}/{k_folds}: Training Loss={train_loss}, Accuracy={train_accuracy}, F1={train_f1_score}"
        )
        print("\n")
        ...
        if val_f1_score > best_f1_score:
            ...
            best_w = w
            best_confusion_matrix = compute_confusion_matrix(
    ...
    mean_train_accuracy = np.mean(
        [metrics["train_accuracy"] for metrics in fold_metrics]
    )
    mean_train_f1_score = np.mean(
        [metrics["train_f1_score"] for metrics in fold_metrics]
    )
...
def process_train_data_only_continous(
    ...
    # Filter to only the selected columns
    structured_array_x_train = rfn.repack_fields(
        structured_array_x_train[selected_features]
    )
